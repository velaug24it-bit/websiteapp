...
import { pushPosOrderApi, getPosOrderStatusApi } from '../services/api';
...
const PosCardSwipeModal = ({
  ...
  // Mode: 'physical' (Razorpay POS Option A) | 'interactive' (On-Screen Simulator)
  const [deviceMode, setDeviceMode] = useState('physical');
  // POS States: 'BILLED_READY' | 'PUSHING_TO_DEVICE' | 'WAITING_DEVICE_SWIPE' | 'SWIPING' | 'ENTER_PIN' | 'AUTHORIZING' | 'APPROVED'
  ...
  // Razorpay Physical Machine Settings (Option A)
  const [terminalId, setTerminalId] = useState(
    () => localStorage.getItem('kadalai_pos_terminal_id') || 'TID_KM_STORE_01'
  );
  ...
  // Save Terminal ID
  const handleSaveTerminalId = () => {
    const cleanId = tempTerminalId.trim() || 'TID_KM_STORE_01';
    setTerminalId(cleanId);
    localStorage.setItem('kadalai_pos_terminal_id', cleanId);
    setIsEditingTerminal(false);
  };

  // Option A: Push Billed Amount to Physical Razorpay POS Machine
  const handlePushToPhysicalMachine = async () => {
    playPosTone(900, 'sine', 0.12);
    setPosState('PUSHING_TO_DEVICE');
    setPushStatusMessage(`Connecting to Razorpay POS Terminal (${terminalId})...`);
    ...
    try {
      ...
      const res = await pushPosOrderApi(payload);

      if (res.data.success) {
        playPosTone(1100, 'sine', 0.1);
        setPosState('WAITING_DEVICE_SWIPE');
        setPushStatusMessage(
          `✅ Bill of ₹${totalAmount.toFixed(2)} sent to Terminal ${terminalId}! Please swipe customer debit card on physical machine.`
        );

        // Start live polling in background for when customer swipes on the physical machine
        if (pollingTimerRef.current) clearInterval(pollingTimerRef.current);
        const orderIdToPoll = res.data.orderId || orderData.razorpayOrderId;

        pollingTimerRef.current = setInterval(async () => {
          try {
            const statusRes = await getPosOrderStatusApi(orderIdToPoll);
            if (statusRes.data.success && statusRes.data.status === 'APPROVED') {
              clearInterval(pollingTimerRef.current);
              playApprovalChime();
        ...
        setPushStatusMessage('Could not push to terminal. Switching to on-screen swipe.');
        setPosState('BILLED_READY');
      }
    } catch (err) {
      console.warn('Physical POS Push error:', err);
      // Fallback: ready on screen
      setPosState('WAITING_DEVICE_SWIPE');
      setPushStatusMessage(
        `📡 Simulated Terminal ${terminalId} active: Tap 'Simulate Physical Swipe Approval' or swipe card below.`
      );
    }
  };

  // Immediate confirmation trigger (for counter staff when customer swipes on physical terminal)
  const handleManualConfirmFromPhysicalTerminal = () => {
    if (pollingTimerRef.current) clearInterval(pollingTimerRef.current);
    playApprovalChime();
    setPosState('APPROVED');
    setShowReceipt(true);
  ...
  const handleFinishAndConfirmOrder = () => {
    ...
    onSuccess({
      ...
      paymentMethod: 'Debit Card (Razorpay POS Machine)',
      posDetails: {
        cardBrand: cardType,
        last4: '4892',
        authCode,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/80 backdrop-blur-sm animate-fadeIn overflow-y-auto">
      <div className="relative w-full max-w-4xl bg-stone-900 text-white rounded-3xl shadow-2xl border border-stone-700 overflow-hidden my-4 max-h-[96vh] flex flex-col">
        {/* Top Header */}
        <div className="bg-gradient-to-r from-stone-900 via-stone-800 to-stone-900 px-4 sm:px-6 py-3.5 border-b border-stone-700 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-emerald-600 flex items-center justify-center font-bold shadow-md shadow-emerald-900/50">
              <CreditCard className="w-5 h-5 text-white" />
            </div>
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <span className="font-bold text-sm tracking-wide text-white">Razorpay POS Device Integration</span>
                <span className="text-[10px] bg-emerald-500/20 text-emerald-300 font-mono px-2 py-0.5 rounded-full border border-emerald-500/30 flex items-center gap-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse"></span> OPTION A READY
                </span>
              </div>
              <p className="text-xs text-stone-400">Physical POS Terminal Push & Digital Counter Billing</p>
            </div>
          </div>
        ...
        {/* Physical Device Connection Status Banner */}
        <div className="bg-stone-950 px-4 sm:px-6 py-2 border-b border-stone-800 flex flex-wrap items-center justify-between text-xs text-stone-300 gap-2">
            ...
            {isEditingTerminal ? (
              <div className="flex items-center gap-1.5">
                <input
                  ...
                  onChange={(e) => setTempTerminalId(e.target.value)}
                  placeholder="e.g. TID_948201"
                  className="px-2 py-0.5 rounded bg-stone-800 border border-stone-600 text-emerald-300 text-xs font-mono"
                />
                <button
                  type="button"
              ...
              <div className="flex items-center gap-1.5">
                <span className="font-mono font-bold text-emerald-300">{terminalId}</span>
                <button
                  type="button"
                  onClick={() => setIsEditingTerminal(true)}
                  className="text-stone-500 hover:text-stone-300"
                  title="Configure Physical Terminal ID"
                >
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={() => setDeviceMode('interactive')}
              className={`px-2.5 py-1 rounded-lg text-[11px] font-bold transition-all ${
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 flex-1 overflow-y-auto">
          {/* LEFT COLUMN: Retail Tax Bill / Invoice */}
          <div
            ...
            <div className="bg-stone-900/90 rounded-2xl p-4 border border-stone-800 text-xs space-y-3 font-sans shadow-inner">
              ...
              <div className="p-2 rounded-xl bg-stone-950 border border-stone-800 text-center">
                <span className="text-[10px] font-mono text-emerald-400 uppercase tracking-wider flex items-center justify-center gap-1">
                  <Check className="w-3 h-3 text-emerald-400" /> Billed & Ready for Terminal Swipe
                </span>
              </div>
            </div>
          ...
          {/* RIGHT COLUMN: Razorpay POS Machine Terminal Screen */}
          <div
            className={`lg:col-span-7 p-4 sm:p-6 bg-gradient-to-b from-stone-900 to-stone-950 flex flex-col items-center justify-center space-y-4 ${
          ...
          >
            ...
            <div className="w-full max-w-md bg-stone-900 rounded-3xl p-5 border-2 border-stone-700 shadow-2xl shadow-black relative overflow-hidden">
              {/* Thermal Printer Paper Output Slot */}
              ...
              <div className="flex items-center justify-between text-[10px] text-stone-400 font-mono pb-2 border-b border-stone-800">
                <span className="flex items-center gap-1 text-emerald-400">
                  <Radio className="w-3 h-3 text-emerald-400 animate-pulse" /> Razorpay POS Cloud
                </span>
                <span className="text-stone-300">TID: {terminalId}</span>
              ...
              <div className="mt-3 bg-[#0a192f] border-2 border-[#1e3a5f] rounded-2xl p-4 text-center font-mono shadow-inner relative overflow-hidden">
                <div className="text-[10px] tracking-wider text-blue-300 uppercase mb-1">
                  Razorpay Smart POS • EMV L2 Certified
                </div>

                <div className="text-xs text-blue-200 uppercase font-semibold">Billed Amount Due</div>
                <div className="text-3xl font-black text-emerald-400 my-1 font-mono tracking-tight">
                  ₹{totalAmount.toFixed(2)}
                </div>

                {/* Dynamic Screen Status Banner */}
                <div className="mt-2 py-2 px-3 rounded-lg bg-black/40 border border-blue-900/50">
                  {posState === 'BILLED_READY' && (
                    <p className="text-xs font-bold text-amber-300 uppercase animate-pulse">
                      👉 READY • PUSH TO DEVICE OR SWIPE CARD
                    </p>
                  )}
                  {posState === 'PUSHING_TO_DEVICE' && (
                    <div className="space-y-1">
                      ...
                      <p className="text-xs font-bold text-amber-300 uppercase">
                        TRANSMITTING BILL TO TERMINAL {terminalId}...
                      </p>
                    </div>
                  )}
                  {posState === 'WAITING_DEVICE_SWIPE' && (
                    <div className="space-y-1">
                      <p className="text-xs font-bold text-emerald-300 uppercase flex items-center justify-center gap-1.5 animate-pulse">
                        <Radio className="w-4 h-4 text-emerald-400" />
                        BILL PUSHED • WAITING FOR SWIPE AT COUNTER
                      </p>
                      <p className="text-[10px] text-blue-300">Customer should insert/swipe debit card on physical machine</p>
                    </div>
                  )}
                  {posState === 'SWIPING' && (
                  ...
                  {posState === 'AUTHORIZING' && (
                    <div className="space-y-1 py-1">
                      ...
                      <p className="text-xs font-bold text-emerald-300 uppercase">
                        AUTHORIZING WITH NPCI & BANK SWITCH...
                      </p>
                    </div>
                  )}
                  {posState === 'APPROVED' && (
              ...
              {/* Status Message Text */}
              {pushStatusMessage && (
                <div className="mt-2 p-2 rounded-xl bg-stone-950/80 border border-stone-800 text-[11px] text-emerald-300 text-center font-mono">
                  {pushStatusMessage}
                </div>
              )}

              {/* Primary Action Buttons depending on Mode */}
              <div className="mt-4 space-y-2">
                {posState === 'BILLED_READY' && deviceMode === 'physical' && (
                  <button
                    type="button"
                    id="pos-push-to-device-btn"
                    onClick={handlePushToPhysicalMachine}
                    className="w-full py-3.5 px-4 rounded-2xl bg-gradient-to-r from-emerald-600 via-teal-600 to-emerald-700 hover:from-emerald-500 hover:to-teal-500 text-white font-bold text-xs shadow-lg shadow-emerald-900/40 transition-all flex items-center justify-center gap-2 active:scale-98"
                  >
                    <Send className="w-4 h-4" />
                    <span>📡 Send Bill to Razorpay POS Machine (₹{totalAmount.toFixed(2)})</span>
                  </button>
                )}
                ...
                {posState === 'WAITING_DEVICE_SWIPE' && (
                  <div className="space-y-2 animate-fadeIn">
                    <button
                      type="button"
                      id="pos-confirm-physical-swipe-btn"
                      onClick={handleManualConfirmFromPhysicalTerminal}
                      className="w-full py-3 px-4 rounded-2xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs shadow-lg flex items-center justify-center gap-2 active:scale-98 transition-all"
                    >
                      <CheckCircle2 className="w-4 h-4" />
                      <span>Card Swiped on Machine • Confirm Payment</span>
                    </button>
                  </div>
                )}
              </div>

              {/* Visual 3D Indian Debit Card Preview (for simulated/counter swipe) */}
              {(deviceMode === 'interactive' || posState === 'ENTER_PIN' || posState === 'SWIPING') && (
                <div className="mt-3">
                  <div
